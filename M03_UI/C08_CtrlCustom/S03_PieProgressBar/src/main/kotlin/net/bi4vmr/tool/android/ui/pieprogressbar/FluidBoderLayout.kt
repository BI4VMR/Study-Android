 ...
 * @version 1.0
 */
class FluidBoderLayout @JvmOverloads constructor(
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        pieRect.set(0F, 0F, w.toFloat(), h.toFloat())
        Log.d("PieProgressBar", "OnSizeChanged. X:[${pieRect.centerX()}] Y:[${pieRect.centerY()}]")

        // 在这里初始化 SweepGradient，此时已经有了正确的中心点坐标
        ...
        val shader = SweepGradient(pieRect.centerX(), pieRect.centerY(), colors, positions)
    ...
    private fun drawBorder(canvas: Canvas) {
        // 设置旋转矩阵
        matrix.reset()
        matrix.setRotate(rotationAngle, pieRect.centerX(), pieRect.centerY())
        (gradientPaint?.shader as? SweepGradient)?.setLocalMatrix(matrix)
    ...
    fun startFluid() {
        ...
        fluidAnim = ObjectAnimator.ofFloat(this, "rotationAngle", 0f, 360f).apply {
            duration = 2000L
