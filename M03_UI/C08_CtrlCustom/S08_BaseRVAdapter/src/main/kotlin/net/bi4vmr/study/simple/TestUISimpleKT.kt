...
import net.bi4vmr.study.databinding.TestuiSimpleBinding
 ...
 * @since 1.0.0
 */
class TestUISimpleKT : AppCompatActivity() {

    companion object {
        private val TAG: String = "TestApp-${TestUISimpleKT::class.java.simpleName}"
    }

    private val binding: TestuiSimpleBinding by lazy {
        TestuiSimpleBinding.inflate(LayoutInflater.from(applicationContext))
    }
    ...
    private fun initView() {
        with(binding) {
            // 准备测试数据
            val dataSource: MutableList<ContentVO> = initTestData()
            // 创建Adapter
            val adapter = TestSingleTypeAdapter(dataSource)
            // 配置RecyclerView
            recyclerview.setLayoutManager(LinearLayoutManager(this@TestUISimpleKT))
    ...
    private fun initTestData(): MutableList<ContentVO> {
